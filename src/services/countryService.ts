...
// 🌐 LISTA EXPANDIDA DE PAÍSES (AMÉRICA, EUROPA, ÁFRICA E ÁSIA)
...
export const getCountryList = (): CountryData[] => {
  const priority = ALL_COUNTRIES.filter((c) => PRIORITY_CODES.includes(c.code));
  const others = ALL_COUNTRIES.filter((c) => !PRIORITY_CODES.includes(c.code)).sort((a, b) =>
